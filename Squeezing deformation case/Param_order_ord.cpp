...
void InitialConditions(int N, double Lx, double Ly, double radius, double a, vector<vector<site>> &sites, vector<particle> &Particles, MTRand &ran);
...
int main(int argc, char *argv[]){ 
 ...
 // init_genrand64(seed);
  ...
  //printf("rho*Lx*Ly %.3f\n", rho*Lx*Ly);
  //printf("floor0(rho*Lx*Ly) %.3f\n", floor0(rho*Lx*Ly));
  //printf("static_cast<int>(floor0(rho*Lx*Ly)) %d\n", static_cast<int>(floor0(rho*Lx*Ly)));
  //N = 10;
  a = sqrt(2/(sqrt(3)*rho));
  ...
  Nx = static_cast<int>(floor(Lx/(2*radius)));
  Ny = static_cast<int>(floor(Ly/(2*radius)));  
  ...
  // Initial conditions
  InitialConditions(N, Lx, Ly, radius, a, sites, Particles,ran);
...
void add_force(int k1, particle p1, int k2, particle p2, double p2x, double p2y, double radius, double lambda, double amplitude_r, double amplitude_phi, double amplitude_theta, double Ly, double Lx, vector<force> &Force){
  ...
  double amp_theta, amp_align; // Interaction strengths
  double sigma0, d1, d2, chi2, tau, tau_, sigma0_2, d1_2, d2_2, radius_2, dr_dx, dr_dy, dpsi_dx, dpsi_dy, l1_2_m_d1_2, l2_2_p_d1_2, l1_2_p_d2_2, l2_2_m_d2_2;
  ...
  double tau_r12u1_2, btau_r12u2_2, chi2u1u2, r12u1r12u2, chi2_1, r12pu1, r12pu2, tau_r12u1, btau_r12u2, dsigma_dtheta, sigma3_sigma0_2, dsigma_dd1, dsigma_dd2;
  double chi2_1_2, u1pu2, r12u1p, dU_dtheta, dsigma_dsigma0, lcos_i;
  double dsigma0_dphi, dtau_dphi, dbtau_dphi, dchi2_dphi, dsigma_dtau, dsigma_dbtau, dsigma_dchi2, dU_dphi;
  double dtau_dphi2, dbtau_dphi2, lcos_j, dchi2_dphi2, k0_1_l, dU_dphi2, u1u2p, r12u2p, dsigma_dtheta2, dU_dtheta2;
  ...
  u2x = cos(p2.theta);
  u2y = sin(p2.theta);
  k0_1_l = radius/(1+lambda); //k0/(1+lambda)
  d1 = k0_1_l*(1 + lambda*sin(p1.phi));
  d2 = k0_1_l*(1 + lambda*sin(p2.phi));
  d1_2 = d1*d1;
  d2_2 = d2*d2;
  l1 = radius;
  l2 = radius;
  ...
  //printf("r: %.3f \n", r);
   ...
   //printf("dU: %.3f \n",U.Utot);
    ...
    dU_dphi = dU_dsigma*dd1_dphi*d1*dsigma_dd1;
...
//printf("dU_dphi: %.3f \n", dU_dphi); 
    ...
    dd2_dphi2 = lcos_j*k0_1_l;

    dtau_dphi2 = dtau_dphi*tau; //-tau*2/(l1^2 + d2^2)

    dbtau_dphi2 = -2/l2_2_p_d1_2;

    dchi2_dphi2 = tau*dbtau_dphi2 + tau_*dtau_dphi2;

    dsigma_dd2 = dsigma_dsigma0*dsigma0_dphi+ sigma3_sigma0_2/(2*chi2_1)*(dsigma_dtau*dtau_dphi2+dsigma_dbtau*dbtau_dphi2+dsigma_dchi2*dchi2_dphi2);

    dU_dphi2 = dU_dsigma*dd2_dphi2*d2*dsigma_dd2;
...
void InitialConditions(int N, double Lx, double Ly, double radius, double a, vector<vector<site>> &sites, vector<particle> &Particles,MTRand &ran){
  ...
  double shift = a/3;
  int Nx = static_cast<int>(floor(Lx/(2*radius)));
  int Ny = static_cast<int>(floor(Ly/(2*radius))); 
...
/*
  //Random initialization
    ...
    Particles[k].phi = genRand(&ran)*2*PI; 
    Particles[k].theta = genRand(&ran)*2*PI;
*/
		// Initialization
