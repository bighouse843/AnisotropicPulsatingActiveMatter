...
void InitialConditions(int N, double Lx, double Ly, double radius, double a, vector<vector<site>> &sites, vector<particle> &Particles, MTRand &ran, double lambda);
...
int main(int argc, char *argv[]){ 
  ...
  */
  ...
  char name_snap[210]; // Name of output file containing snapshots
 ...
 // init_genrand64(seed);
  ...
  Nx = static_cast<int>(floor(Lx/(2*radius*(1+lambda)/(1-lambda))));
  Ny = static_cast<int>(floor(Ly/(2*radius*(1+lambda)/(1-lambda))));  
  if(Nx<3){
    printf("Nx < 3 !");
    exit(1);
  }
  ...
  // Initial conditions
  InitialConditions(N, Lx, Ly, radius, a, sites, Particles,ran, lambda);
                  ...
                  // Current=0;
                    for(i=0;i<N;i++){
                        //	 Current += omega + mu_phi*Force[i].torque;
                          for(j=i;j<N;j++)   {   
                            dummy_phi += cos(Particles[i].phi-Particles[j].phi);
...
void add_force(int k1, particle p1, int k2, particle p2, double p2x, double p2y, double radius, double lambda, double amplitude_r, double amplitude_phi, double amplitude_theta, double Ly, double Lx, vector<force> &Force){
  ...
  double sigma, sigma6, rx, ry, r, r6, u1x, u1y, u2x, u2y, r12x, r12y, k0; // Interparticle distance
  double amp_rep, amp_phi, amp_theta, amp_align; // Interaction strengths
  double sigma0, d1, d2, chi2, alpha2, tau, tau_, sigma0_2, d1_2, d2_2, radius_2, dr_dx, dr_dy, dpsi_dx, dpsi_dy, l1_2_m_d1_2, l2_2_p_d1_2, l1_2_p_d2_2, l2_2_m_d2_2, l1, l2, l1_2, l2_2, d1_2_p_d2_2;
  double r12u1, r12u2, u1u2, r12u1_2, r12u2_2, u1u2_2, sigma3, dU_dr, dU_dpsi, dU_dx, dU_dy, dU_dsigma, dsigma_dpsi, psi;
  double tau_r12u1_2, btau_r12u2_2, chi2u1u2, r12u1r12u2, chi2_1, r12pu1, r12pu2, tau_r12u1, btau_r12u2, dsigma_dtheta, sigma3_sigma0_2, dsigma_dsigma1, dsigma1_dr12u1, dsigma1_du1u2, dsigma1_dr12u2;
  ...
  double dtau_dphi2, dbtau_dphi2, lcos_j_lsin_j_1, lcos_j, lsinj_1_2_l_1_2_2, lsini_l_2, sini, sini_1, dchi2_dphi2, dsigma0_dphi2, k0_1_l, dU_dphi2, u1u2p, r12u2p, dsigma_dtheta2, dU_dtheta2;
  ...
  u2x = cos(p2.theta);
  u2y = sin(p2.theta);
  k0_1_l = k0/(1-lambda); //k0/(1-lambda)
  l1 = k0_1_l*(1 + lambda*sin(p1.phi));
  l2 = k0_1_l*(1 + lambda*sin(p2.phi));
  d1_2 = d1*d1;
  d2_2 = d2*d2;
  l1_2 = l1*l1;
  l2_2 = l2*l2;
  ...
  r12u2_2 = r12u2*r12u2;
  u1u2_2 = u1u2*u1u2;
  tau_r12u1_2 = tau*r12u1_2; 
  btau_r12u2_2 = tau_*r12u2_2; 
  chi2u1u2 = chi2*u1u2; 
  r12u1r12u2 = r12u1*r12u2; 
  ...
  sigma = sigma0/sqrt(1-(tau_r12u1_2+btau_r12u2_2-2*chi2u1u2*r12u1r12u2)/chi2_1);
   ...
   //printf("dU: %.3f \n",U.Utot);
    ...
    dsigma_dtau = r12u1_2;
    dtau_dphi = lcos_i*k0_1_l*2*l1*d1_2_p_d2_2/(l1_2_p_d2_2*l1_2_p_d2_2);


    dsigma_dchi2 = (tau_r12u1_2*u1u2_2 + btau_r12u2_2*u1u2_2-2*r12u1r12u2*u1u2)/chi2_1;
    dchi2_dphi = dtau_dphi*tau_;

    dU_dphi = dU_dsigma*sigma3_sigma0_2/(2*chi2_1)*(dsigma_dtau*dtau_dphi+dsigma_dchi2*dchi2_dphi);
    ...
    dsigma_dbtau = r12u2_2;
    dbtau_dphi2 = lcos_j*k0_1_l*2*l2*d1_2_p_d2_2/(l2_2_p_d1_2*l2_2_p_d1_2);

    dchi2_dphi2 = dbtau_dphi2*tau;

    dU_dphi2 = dU_dsigma*sigma3_sigma0_2/(2*chi2_1)*(dsigma_dbtau*dbtau_dphi2+dsigma_dchi2*dchi2_dphi2);
...
void InitialConditions(int N, double Lx, double Ly, double radius, double a, vector<vector<site>> &sites, vector<particle> &Particles,MTRand &ran, double lambda){
  ...
  while(k<N){
    ...
    Particles[k].phi = -PI/2; 
    Particles[k].theta = PI/6;


    //if(k==0){
    //Particles[k].phi = 0;
    //Particles[k].theta = PI;
    //}
    //else{
    //Particles[k].phi = 0;
    //Particles[k].theta = -PI;     
